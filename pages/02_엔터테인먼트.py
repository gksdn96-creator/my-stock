...
st.set_page_config(
    page_title="엔터테인먼트 주식 분석",
    page_icon="🎬",
    layout="wide"
)

st.title("🎬 엔터테인먼트 주식 전문 분석")
st.caption("스트리밍 · 미디어 · 게임 · 한류 엔터 핵심 기업 심층 분석 대시보드")
...
ENTERTAINMENT_TICKERS = {
    "Disney (미디어/테마파크)": "DIS",
    "Netflix (스트리밍)": "NFLX",
    "Warner Bros Discovery (미디어)": "WBD",
    "Comcast (미디어/케이블)": "CMCSA",
    "Paramount (미디어)": "PARA",
    "Spotify (음악스트리밍)": "SPOT",
    "Live Nation (라이브공연)": "LYV",
    "Electronic Arts (게임)": "EA",
    "Take-Two Interactive (게임)": "TTWO",
    "Roblox (메타버스/게임)": "RBLX",
    "하이브 (K-POP)": "352820.KS",
    "SM엔터테인먼트 (K-POP)": "041510.KQ",
    "JYP Ent. (K-POP)": "035900.KQ",
    "YG엔터테인먼트 (K-POP)": "122870.KQ",
    "CJ ENM (미디어/콘텐츠)": "035760.KQ",
}
...
selected_names = st.sidebar.multiselect(
    "분석할 엔터테인먼트 기업",
    options=list(ENTERTAINMENT_TICKERS.keys()),
    default=["Disney (미디어/테마파크)", "Netflix (스트리밍)", "Spotify (음악스트리밍)",
             "하이브 (K-POP)", "Electronic Arts (게임)"]
)
...
tickers = [ENTERTAINMENT_TICKERS[n] for n in selected_names]
name_map = {ENTERTAINMENT_TICKERS[n]: n for n in selected_names}
...
for i, ticker in enumerate(valid_tickers):
    ...
    with cols[i]:
        ...
        if market_cap:
            st.caption(f"시가총액: ${market_cap/1e9:,.1f}B" if market_cap > 1e9 else f"시가총액: ${market_cap/1e6:,.1f}M")
...
for ticker in valid_tickers:
    ...
    valuation_rows.append({
        ...
        "시가총액($B)": round(safe_get(info, "marketCap", 0) / 1e9, 2) if safe_get(info, "marketCap") else None,
        ...
        "부채비율": round(safe_get(info, "debtToEquity"), 1) if safe_get(info, "debtToEquity") else None,
    })

valuation_df = pd.DataFrame(valuation_rows).set_index("기업")
st.dataframe(valuation_df, use_container_width=True)

st.caption("💡 엔터테인먼트 업종은 콘텐츠 제작비·마케팅비 부담으로 **영업이익률 변동성이 크고**, 스트리밍 기업은 아직 성장 초기 단계라 **매출성장률**이 중요한 지표입니다. 미디어 기업은 콘텐츠 자산 관련 **부채비율**도 함께 체크할 필요가 있습니다.")
...
with col2:
    st.subheader("매출성장률 비교")
    rg_df = valuation_df.reset_index().dropna(subset=["매출성장률(%)"])
    if not rg_df.empty:
        fig_rg = px.bar(rg_df, x="기업", y="매출성장률(%)", color="기업",
                        template="plotly_white")
        fig_rg.update_layout(showlegend=False, height=400)
...
st.caption("우상단에 위치할수록 '고성장 + 고수익성' 기업, 좌하단은 콘텐츠 투자 부담이 큰 성장 초기 기업일 가능성")
